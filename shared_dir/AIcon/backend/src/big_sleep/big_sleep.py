...
def create_text_path(text=None, img=None, encoding=None):
    ...
    if img is not None:
        if isinstance(img, str):
            img_name = "".join(
                img.split(".")[:-1]
            )  # replace spaces by underscores, remove img extension
...
def rand_cutout(image, size, center_bias=False, center_focus=2):
    ...
    if center_bias:
        ...
        offset_x = (
            ...
            if (offset_x > max_offset or offset_x < min_offset)
            else offset_x
        )
        offset_y = (
            random.randint(min_offset, max_offset)
            if (offset_y > max_offset or offset_y < min_offset)
            else offset_y
        )
    else:
...
class Latents(torch.nn.Module):
    def __init__(
        ...
        self.cls = torch.nn.Parameter(
            torch.zeros(num_latents, num_classes).normal_(mean=-3.9, std=0.3)
        )
        self.register_buffer("thresh_lat", torch.tensor(1))

        assert (
            not exists(max_classes) or max_classes > 0 and max_classes <= num_classes
...
class Model(nn.Module):
    def __init__(
        self, image_size, max_classes=None, class_temperature=2.0, ema_decay=0.99
    ):
        super().__init__()
...
class BigSleep(nn.Module):
    ...
    def sim_txt_to_img(self, text_embed, img_embed, text_type="max"):
        ...
        return (
            sign
            * self.loss_coef
            * torch.cosine_similarity(text_embed, img_embed, dim=-1).mean()
        )
    ...
    def forward(self, text_embeds, text_min_embeds=[], return_loss=True):
        ...
        lat_loss = (
            torch.abs(1 - torch.std(latents, dim=1)).mean()
        ...
        cls_loss = (
            (50 * torch.topk(soft_one_hot_classes, largest=False, dim=1, k=999)[0]) ** 2
        ).mean()
...
class Imagine(nn.Module):
    def __init__(
        ...
        *,
        ...
        super().__init__()
        logger = setup_logger()
        logger.info(f"Setup Imagine...")

        if torch_deterministic:
            assert (
                not bilinear
            ), "the deterministic (seeded) operation does not work with interpolation (PyTorch 1.7.1)"
        ...
        if exists(seed):
            logger.info(f"Setting seed of {seed}")
        ...
        self.sess_path = "./sess_abt/" + self.hash_id + ".sess"

        logger.info(f"Hash: {self.hash_id}")
    ...
    def create_clip_encoding(self, text=None, img=None, encoding=None):
        ...
        #    encoding = self.update_story_encoding(epoch=0, iteration=1)
            ...
            encoding = (
                self.create_text_encoding(text) + self.create_img_encoding(img)
            ) / 2
    ...
    def encode_multiple_phrases(self, text, img=None, encoding=None, text_type="max"):
        if text is not None and "|" in text:
            self.encoded_texts[text_type] = [
                self.create_clip_encoding(text=prompt_min, img=img, encoding=encoding)
                for prompt_min in text.split("|")
            ]
        else:
            self.encoded_texts[text_type] = [
    ...
    def set_clip_encoding(self, text=None, img=None, encoding=None, text_min=""):
        ...
        if len(text_min) > 0:
            text = (
                text + "_wout_" + text_min[:255]
                if text is not None
                else "wout_" + text_min[:255]
            )
        ...
        if (i + 1) % self.save_every == 0:
            with torch.no_grad():
                ...
                if self.save_progress:
                    ...
                    save_image(image, Path(f"{self.save_dir}/{str(num).zfill(6)}.png"))

                if self.save_best and top_score.item() < self.current_best_score:
                    self.current_best_score = top_score.item()
                    save_image(
                        image, Path(f"{self.save_dir}/{str(num).zfill(6)}.best.png")
                    )
        ...
        for epoch in range(self.epochs):
            for i in range(self.iterations):
                ...
                if not os.path.exists(self.sess_path):
                    ...
                    return True
                out, loss = self.train_step(epoch, i)
