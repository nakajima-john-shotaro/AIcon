...
class Imagine(nn.Module):
    ...
    def image_output_path(self, sequence_number=0):
        """
        ...
        if sequence_number:
            ...
            output_path = f"{sequence_number_left_padded}"
        if self.save_date_time:
            current_time = datetime.now().strftime("%y%m%d-%H%M%S_%f")
            output_path = f"{current_time}_{output_path}"
        return Path(f"{self.save_dir}/{output_path}.jpg")
    ...
    def generate_gif(self):
        images = []
        for file_name in sorted(os.listdir(f'{self.save_dir}/')):
            if file_name.startswith('00') and file_name.endswith('jpg'):
                images.append(imread(os.path.join(self.save_dir, file_name)))

        if self.save_video:
            mimsave(f'{self.save_dir}/{self.textpath}.mp4', images)
            print(f'Generated image generation animation at {self.save_dir}/{self.textpath}.mp4')
        if self.save_gif:
            mimsave(f'{self.save_dir}/{self.textpath}.gif', images)
            print(f'Generated image generation animation at {self.save_dir}/{self.textpath}.gif')
